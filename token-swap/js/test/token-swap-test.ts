...
const TOKEN_SWAP_PROGRAM_ID = new PublicKey(
  "EHWNmhGTTTrd7sSacuwEZkS5scTC3rTiVi1Tp2VY6rj8"
  );
...
export async function createTokenSwap(
  ...
  const secretKey = JSON.parse(fs.readFileSync("/Users/dboures/.config/solana/id.json", 'utf8'))
  const payer = Keypair.fromSecretKey(Uint8Array.from(secretKey))


  console.log('1');
  const owner = await newAccountWithLamports(connection, 1000000000, payer);
  console.log('2');
  const swapPayer = await newAccountWithLamports(connection, 1000000000, payer);
